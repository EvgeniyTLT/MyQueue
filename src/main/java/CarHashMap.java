...
public class CarHashMap implements CarMap {
    ...
    private static final int INITIATL_CAPACITY = 16;
    private static final double LOAD_FACTOR = 0.75;
    private Entry[] array = new Entry[INITIATL_CAPACITY];
    private int size = 0;
    ...
    public Set<CarOwner> keySet() {
        ...
        for (Entry entry : array) {
            Entry existenElement = entry;
            while (existenElement != null) {
                result.add(existenElement.key);
    ...
    public List<Car> values() {
        ...
        for (Entry entry : array) {
            Entry existenElement = entry;
            while (existenElement != null) {
                result.add(existenElement.value);
                existenElement = existenElement.next;
            }
    ...
    @Override
    public void clear() {
        array = new Entry[INITIATL_CAPACITY];
        size = 0;
    }
    ...
    private void increaseArray() {
        ...
        for (Entry entry : array) {
            Entry existenElement = entry;
            while (existenElement != null) {
                put(existenElement.key, existenElement.value, newArray);
                existenElement = existenElement.next;
            }
        }
        array = newArray;
